...
static int xilinx_vdma_probe(struct platform_device *pdev)
{
   ...
   /************************ config VDMA2    **********************************/
    ...
    XAxiVdma_WriteReg(VDMA2_virtual_address, S2MM_START_ADDRESS0  ,  pFrames_phys[0] );   // FOR quick test set to pFrames_phys[0] for direct display  S2MM_START_ADDRESS
    XAxiVdma_WriteReg(VDMA2_virtual_address, S2MM_START_ADDRESS1  ,  pFrames_phys[0] );   //  S2MM_START_ADDRESS
    XAxiVdma_WriteReg(VDMA2_virtual_address, S2MM_START_ADDRESS2  ,  pFrames_phys[0] );   //   S2MM_START_ADDRESS
    // set S2MM parking frame index
    readRegValue = XAxiVdma_ReadReg(VDMA2_virtual_address,PARK_PTR_REG);
    readRegValue = readRegValue & (~S2MM_PARK_BIT_MASK)  ;  // keep the other bits , clear the corresponding bits
    ...
    XAxiVdma_WriteReg(VDMA3_virtual_address, S2MM_START_ADDRESS0  ,  pFrames_phys[0] + 640 * 4 );   //   S2MM_START_ADDRESS
    XAxiVdma_WriteReg(VDMA3_virtual_address, S2MM_START_ADDRESS1  ,  pFrames_phys[0] + 640 * 4 );   //  S2MM_START_ADDRESS
    XAxiVdma_WriteReg(VDMA3_virtual_address, S2MM_START_ADDRESS2  ,  pFrames_phys[0] + 640 * 4 );   //   S2MM_START_ADDRESS
    // set S2MM parking frame index
    readRegValue = XAxiVdma_ReadReg(VDMA3_virtual_address,PARK_PTR_REG);
    readRegValue = readRegValue & (~S2MM_PARK_BIT_MASK)  ;  // keep the other bits , clear the corresponding bits
