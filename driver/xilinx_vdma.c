	...
	if (chan->has_sg) {
		...
		list_for_each_entry(segment, &desc->segments, node) {
			...
			vdma_desc_write(chan,
					XILINX_VDMA_REG_START_ADDRESS(i++),
					(u32)segment->hw.buf_addr);                         // set the start address of the 3 frame buffers 

            printk("vdma buffer address  = %d \n", (u32)segment->hw.buf_addr) ;  // ????? controll register address of buff address
		...
		vdma_desc_write(chan, XILINX_VDMA_REG_FRMDLY_STRIDE,
				last->hw.stride);
		vdma_desc_write(chan, XILINX_VDMA_REG_VSIZE, last->hw.vsize);  // writing this vsize parameter will acturally start or change the settings 
	}
...
/*
Question :
	1. how do I know when the read buffer is ready and which buffer to read , and which write buffer to write ? 
	2. how do I open the device node to reconfig the parameters ?  
*/
